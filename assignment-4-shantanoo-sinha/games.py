...
#         v = max_value(game.result(state, a), best_score, beta)
#         if v < best_score:
#             best_score = v
#             best_action = a
    if player == 'X':
        global_var.utility_for_X = best_score
    else:
        global_var.utility_for_X = -best_score
    
    global_var.alphabeta_states_evaluated = states_evaluated
    return best_action
...
#     def utility_for_X(self, state, player):
#         """Return the value to player; 1 for win, -1 for loss, 0 otherwise."""
#         return state.utility if player == 'X' else -state.utility

    def terminal_test(self, state):
        """A state is terminal if it is won or there are no empty squares."""
#         print("Terminal Test", state.utility, len(state.moves))
